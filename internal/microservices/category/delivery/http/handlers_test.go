...
func TestHandler_GetTags(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			user:         user,
			expectedCode: http.StatusOK,
			expectedBody: `{"status":200,"body":[{"id":"` + uuidTest.String() + `","user_id":"` + uuidTest.String() + `","parent_id":"` + uuid.Nil.String() + `","image_id":0,"name":"TestTag","show_income":true,"show_outcome":true,"regular":true}]}`,
			mockUsecaseFn: func(mockUsecase *mocks.MockCategoryServiceClient) {
...
func TestHandler_UpdateTag(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			tag:          tag,
			expectedCode: http.StatusOK,
			expectedBody: `{"status":200,"body":{"id":"` + tagID.String() + `","user_id":"` + uuidTest.String() + `","parent_id":"` + uuid.Nil.String() + `","image_id":10,"name":"TestTag","show_income":true,"show_outcome":true,"regular":true}}`,
			mockUsecaseFn: func(mockUsecase *mocks.MockCategoryServiceClient) {

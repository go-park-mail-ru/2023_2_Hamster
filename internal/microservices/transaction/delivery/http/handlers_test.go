...
func TestHandler_GetFeed(t *testing.T) {
	...
	tests := []struct {
		...
		expectedBody  string
		mockUsecaseFn func(*mocks.MockUsecase)
		mockUserFn    func(*mockUser.MockUsecase)
	}{
		{
			...
			expectedCode: http.StatusOK,
			expectedBody: `{"status":200,"body":{"transactions":[{"id":"00000000-0000-0000-0000-000000000000","account_income":"00000000-0000-0000-0000-000000000000","account_outcome":"00000000-0000-0000-0000-000000000000","income":0,"login":"login","outcome":0,"date":"0001-01-01T00:00:00Z","payer":"","description":"","categories":null}]}}`,
			mockUsecaseFn: func(mockUsecase *mocks.MockUsecase) {
				mockUsecase.EXPECT().GetFeed(gomock.Any(), gomock.Any(), gomock.Any()).Return([]models.Transaction{{UserID: uuidTest}}, nil)
			},
			mockUserFn: func(mu *mockUser.MockUsecase) {
				mu.EXPECT().GetUser(gomock.Any(), gomock.Any()).Return(&models.User{ID: uuidTest, Login: "login", Username: "username"}, nil)
			},
		},
		{
			name:          "Unauthorized Request",
